...
# Solidity: What and Why

## What is Solidity

Solidity is an object-oriented language designed to create scripts called "smart contracts" on the Ethereum Virtual Machine, which runs on the Ethereum Blockchain.
...
Solidity is the most popular language which can compile into EVM bytecode. Using Solidity, we can create scripts that run on the Ethereum Blockchain to create any program. Due to the blockchain's properties, programs that focus on scarcity, social coordination, ownership, or finance are popular use cases.

## Who should learn Solidity

Those that are interested in web3 decentralized application (DApp) development. If you wish to explore this new field of human coordination through computer networks, Solidity is a great place to start. Web2 focuses on optimized algorithms to deliver results. Web3 focuses on mechanisms to deliver results.

With Solidity, developers can create entities that rely on networks to produce goods and services instead of top-down approaches or decentralization.
...
Through mechanisms developers can create bottom up networks, reducing the need for central coordinators by relying on markets and various emergent methods of collaboration at scale.
